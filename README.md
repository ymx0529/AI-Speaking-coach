...
- `Qwen / 自定义 OpenAI 兼容模型`：语法、表达、词汇纠错
...
```powershell
cd D:\Desktop\QiNiu\backend; conda run -n qniu uvicorn app.main:app --reload --port 8000
```

### 前端一条命令

```powershell
cd D:\Desktop\QiNiu\frontend; npm.cmd run dev
```
...
- `.env.example` 仅用于示例，不应提交真实密钥。
- 如果系统 `PATH` 中没有 `ffmpeg`，请在 `.env` 中配置 `FFMPEG_BINARY`。
- 发音评测可按环境切换为 `xfyun`、`azure` 或 `auto`。
- 当前项目以英文口语训练为目标，界面文案为中文，AI 对话内容为英文。
